...
public class TestCategoryImpl {
    ...
    public void testGetAndAddCategory() {
        ...
        spaceDao.addCategory(category);
        List<Category> allCategories = spaceDao.getAllCategories();


        assertNotNull("List should never be null", allCategories);
        assertEquals("should only have 5 categories in list",5 , allCategories.size());

        Category testCategory = spaceDao.getCategory(category.getId());
        assertEquals("should equal each other", category1, testCategory);
    ...
    public void testUpdateCategory() {
        ...
        List<Category> allCategories = spaceDao.getAllCategories();
        assertTrue("List should contain category", allCategories.contains(category));

        Category toEdit = spaceDao.getCategory(category.getId());
        assertEquals("they should equal each other", category, toEdit);
        //get organization out and edit it


        category.setName("Tesla");
        
        assertNotEquals("category should not equal each other after changes", category, toEdit);
        spaceDao.updateCategory(category);

        assertNotEquals(category, toEdit);

        toEdit = spaceDao.getCategory(category.getId());

        assertEquals("after updating category they should equal each other", category, toEdit);
    }
    @Test
    public void testRemoveCategory() {
        Category category= category1;
        Category categoryTwo = category2;
        spaceDao.addCategory(category);
        spaceDao.addCategory(categoryTwo);

        List<Category> allCategories = spaceDao.getAllCategories();
        assertTrue("list should be contain both category",allCategories.contains(category));
        assertTrue("list should be contain both categoryTwo",allCategories.contains(categoryTwo));

        Category fromDao = spaceDao.getCategory(category.getId());

        assertEquals("both objects should equal each other",category, fromDao);

        spaceDao.removeCategory(category.getId());
        allCategories = spaceDao.getAllCategories();

        assertEquals("list should be contain only CategorTwo",5,allCategories.size());
        assertTrue("list should be contain CategoryTwo",allCategories.contains(categoryTwo));
