...
public class TestStatusImpl {
    ...
    @Test
    public void testAddAndGetStatus() {

        Status status = testStatuses[0];

        spaceDao.addStatus(status);
        List<Status> allStatuss = spaceDao.getAllStatuses();

        assertNotNull("List should never be null", allStatuss);
        assertEquals("should only have 4 statuses in list",5 , allStatuss.size());
        Status teststatus= spaceDao.getStatus(status.getId());
        assertEquals("should equal each other", status,teststatus );
    ...
    public void testGetAndUpdateStatus() {
        ...
        Status status = testStatuses[1];



        List<Status> allPowers = spaceDao.getAllStatuses();
        assertTrue("List should contain hero", allPowers.contains(status));

        Status toEdit = spaceDao.getStatus(status.getId());
        assertEquals("they should equal each other", status, toEdit);
        //get organization out and edit it


        status.setName("Under Review");
        assertNotEquals("status should not equal each other after changes", status, toEdit);
        spaceDao.updateStatus(status);

        assertNotEquals(status, toEdit);

        toEdit = spaceDao.getStatus(status.getId());

        assertEquals("after updating status they should equal each other", status, toEdit);
    }

    @Test
    public void testRemoveStatus() {
        Status status = testStatuses[1];
        Status status2 = testStatuses[0];
        spaceDao.addStatus(status);
        spaceDao.addStatus(status2);

        List<Status> allStatuses = spaceDao.getAllStatuses();
        assertTrue("list should be contain both statuses",allStatuses.contains(status));
        assertTrue("list should be contain both statuses",allStatuses.contains(status2));

        Status fromDao = spaceDao.getStatus(status.getId());

        assertEquals("both objects should equal each other",status, fromDao);

        spaceDao.removeStatus(status.getId());
        allStatuses = spaceDao.getAllStatuses();


        assertEquals("list should be contain only status2",5,allStatuses.size());
        assertTrue("list should be contain both statuses",allStatuses.contains(status2));
