...
public class TestHashtagImpl {
    ...
    public void setUp() {
        ...
        public void testAddAndGetHashtag() {
            ...
            Hashtag hashtag = hashtag1;

            spaceDao.addHashtag(hashtag);
            List<Hashtag> allHashtags = spaceDao.getAllHashtags();

            assertNotNull("List should never be null", allHashtags);
            assertEquals("should only have 4 organization in list", 9, allHashtags.size());
            Hashtag testHash = spaceDao.getHashtag(hashtag.getId());
            assertEquals("should equal each other", hashtag, testHash);

        }

        @Test
        public void catchAlreadyCaughtHashtag() {

        Hashtag hashtag = hashtag1;
        Hashtag hashtagtwo = new Hashtag(10,"It Worked");
        Collection<Hashtag> list = new ArrayList<>(spaceDao.getAllHashtags());
        Set<Hashtag> allHashtags = new HashSet<>(list);
        if(!allHashtags.contains(hashtag)){
                spaceDao.addHashtag(hashtag);

            }
            if(!allHashtags.contains(hashtagtwo)){
                spaceDao.addHashtag(hashtagtwo);

            }
            list = new ArrayList<>(spaceDao.getAllHashtags());
            allHashtags = new HashSet<>(list);
        assertNotNull("List should never be null", allHashtags);
        assertEquals("should only have 4 organization in list", 9, allHashtags.size());
        Hashtag testHash = spaceDao.getHashtag(hashtagtwo.getId());
        assertEquals("should equal each other", hashtagtwo, testHash);
        ...
        @Test
        public void testGetAllAndUpdateHashtag() {
            Hashtag hashtag = testHashtags[0];


            List<Hashtag> allHashtags = spaceDao.getAllHashtags();
            assertTrue("List should contain hashtag", allHashtags.contains(hashtag));

            Hashtag toEdit = spaceDao.getHashtag(hashtag.getId());
            assertEquals("they should equal each other", hashtag, toEdit);
            //get organization out and edit it


            hashtag.setName("#babylonFive");
            assertNotEquals("hashtag should not equal each other after changes", hashtag, toEdit);
            spaceDao.updateHashtag(hashtag);

            assertNotEquals(hashtag, toEdit);

            toEdit = spaceDao.getHashtag(hashtag.getId());

            assertEquals("after updating hashtag they should equal each other", hashtag, toEdit);
        }

        @Test
        public void testRemoveHashtag() {
            Hashtag hashtag = testHashtags[1];
            Hashtag hashtag2 = testHashtags[0];
            spaceDao.addHashtag(hashtag);
            spaceDao.addHashtag(hashtag2);

            List<Hashtag> allHashtags = spaceDao.getAllHashtags();
            assertTrue("list should be contain hashtag", allHashtags.contains(hashtag));
            assertTrue("list should be contain hashtag2 ", allHashtags.contains(hashtag2));

            Hashtag fromDao = spaceDao.getHashtag(hashtag.getId());

            assertEquals("both objects should equal each other", hashtag, fromDao);

            spaceDao.removeHashtag(hashtag.getId());
            allHashtags = spaceDao.getAllHashtags();

            assertEquals("list should be contain hashtag2", 9, allHashtags.size());
            assertTrue("list should be contain contain hashtag2", allHashtags.contains(hashtag2));
