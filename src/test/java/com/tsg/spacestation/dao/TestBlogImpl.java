...
public class TestBlogImpl {
    ...
    public void setUp() {
        ...
        heySQL.update(INSERT_INTO_BLOGCAT, 2, 2, 2);
        heySQL.update(INSERT_INTO_BLOGCAT, 3, 3, 2);
    ...
    public void testAddAndGetBlog() {
        ...
        Blog blog = testBlogs[0];
        blog.setCategoryId(1);
        spaceDao.addBlog(blog);

//        List<Blog> allBlogs = spaceDao.getAllBlogs();
//
//        assertNotNull("List should never be null", allBlogs);
//        assertEquals("should only have 4 bloges in list",4 , allBlogs.size());
        Blog testblog= spaceDao.getBlog(blog.getId());
        assertEquals("should equal each other", blog,testblog );
    ...
    public void testGetAndUpdateBlog() {
        ...
        Blog blog = testBlogs[1];
        spaceDao.addBlog(blog);
        List<Blog> allBlogs = spaceDao.getAllBlogs();
        assertTrue("List should contain blog", allBlogs.contains(blog));
        
        Blog toEdit = spaceDao.getBlog(blog.getId());
        assertEquals("they should equal each other", blog, toEdit);
        //get organization out and edit it

        blog.setTitle("Elon Husk who???");
        assertNotEquals("blog should not equal each other after changes", blog, toEdit);
        spaceDao.updateBlog(blog);
        
        assertNotEquals(blog, toEdit);
        
        toEdit = spaceDao.getBlog(blog.getId());
        
        assertEquals("after updating blog they should equal each other", blog, toEdit);
    }
    
    @Test
    public void testRemoveBlog() {
        Blog blog = testBlogs[1];
        Blog blog2 = testBlogs[0];
        spaceDao.addBlog(blog);
        spaceDao.addBlog(blog2);
        
        List<Blog> allBlogs = spaceDao.getAllBlogs();
        assertTrue("list should be contain both blogs", allBlogs.contains(blog));
        assertTrue("list should be contain both blogs", allBlogs.contains(blog2));
        
        Blog fromDao = spaceDao.getBlog(blog.getId());
        
        assertEquals("both objects should equal each other", blog, fromDao);
        
        spaceDao.removeBlog(blog.getId());
        allBlogs = spaceDao.getAllBlogs();
        
        assertEquals("list should be contain only blog2", 4, allBlogs.size());
        assertTrue("list should be contain  blog2", allBlogs.contains(blog2));
