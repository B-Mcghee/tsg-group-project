...
public class TestImageImpl {
    ...
    @Test
    public void testGetAndAddImage() {

       Image image = image1;

        spaceDao.addImage(image);
        List<Image> allImages = spaceDao.getAllImages();


        assertNotNull("List should never be null", allImages);
        assertEquals("should only have 5 images in list",4 , allImages.size());

        Image testImage = spaceDao.getImage(image.getId());
        assertEquals("should equal each other", image1, testImage);

    }

    @Test
    public void testGetAllAndUpdateImage() {


        Image image = image1;

        List<Image> allImages = spaceDao.getAllImages();
        assertTrue("List should contain image", allImages.contains(image));

        Image toEdit = spaceDao.getImage(image.getId());
        assertEquals("they should equal each other", image, toEdit);
        //get organization out and edit it


        image.setType("profile picture");

        assertNotEquals("image should not equal each other after changes", image, toEdit);
        spaceDao.updateImage(image);

        assertNotEquals(image, toEdit);

        toEdit = spaceDao.getImage(image.getId());

        assertEquals("after updating image they should equal each other", image, toEdit);
    }
    @Test
    public void testRemoveImage() {
        Image image = image1;
        Image otherImage = image2;
        spaceDao.addImage(image);
        spaceDao.addImage(otherImage);

        List<Image> allImages = spaceDao.getAllImages();
        assertTrue("list should be contain both image1",allImages.contains(image));
        assertTrue("list should be contain both otherImage",allImages.contains(otherImage));

        Image fromDao = spaceDao.getImage(image.getId());

        assertEquals("both objects should equal each other",image, fromDao);

        spaceDao.removeImage(image.getId());
        allImages = spaceDao.getAllImages();

        assertEquals("list should be contain only image1",4,allImages.size());
        assertTrue("list should be contain otherImage",allImages.contains(otherImage));
