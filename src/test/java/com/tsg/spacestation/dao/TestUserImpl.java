...
public class TestUserImpl {
    ...
    public void testAddAndGetUser() {
        ...
        spaceDao.addUser(user);
        List<User> allUsers = spaceDao.getAllUsers();

        assertNotNull("List should never be null", allUsers);
        assertEquals("should only have 4 users in list",5 , allUsers.size());
        User testuser= spaceDao.getUser(user.getId());
        assertEquals("should equal each other", user,testuser );
    ...
    public void testGetAndUpdateUser() {
        ...
        User user = testUsers[1];



        List<User> allUsers = spaceDao.getAllUsers();
        assertTrue("List should contain user", allUsers.contains(user));

        User toEdit = spaceDao.getUser(user.getId());
        assertEquals("they should equal each other", user, toEdit);
        //get organization out and edit it


        user.setDisplayName("TeslaMaster");
        assertNotEquals("user should not equal each other after changes", user, toEdit);
        spaceDao.updateUser(user);

        assertNotEquals(user, toEdit);

        toEdit = spaceDao.getUser(user.getId());

        assertEquals("after updating users they should equal each other", user, toEdit);
    }

    @Test
    public void testRemoveUser() {
        User user = testUsers[1];
        User user2 = testUsers[0];
        spaceDao.addUser(user);
        spaceDao.addUser(user2);

        List<User> allUsers = spaceDao.getAllUsers();
        assertTrue("list should be contain both users",allUsers.contains(user));
        assertTrue("list should be contain both users",allUsers.contains(user2));

        User fromDao = spaceDao.getUser(user.getId());

        assertEquals("both objects should equal each other",user, fromDao);

        spaceDao.removeUser(user.getId());
        allUsers = spaceDao.getAllUsers();

        assertEquals("list should be contain only user2",5,allUsers.size());
        assertTrue("list should be contain  user2",allUsers.contains(user2));
